    ...
    <v-card  :style="{'background': 'rgba(0,0,0,0.3)'}" class="max-card-height min-card-height white--text" >
         <v-card-title class="justify-center">
            ...
            <v-layout class="margin-5">
                ...
                <v-flex v-if="etaTime[1]" class="text-overflow" :class="{'xs12': etaTime[1] != null}"> <h2 >{{etaTime[1]}} min</h2> </v-flex>
                <v-flex v-if="etaTime[2]" class="text-overflow" :class="{'xs12': etaTime[1] != null}"> <h2 >{{etaTime[2]}} min</h2> </v-flex>
            </v-layout>
        </v-card>
    </v-flex>
</template>
